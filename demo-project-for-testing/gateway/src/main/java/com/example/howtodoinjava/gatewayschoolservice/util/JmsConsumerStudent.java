...
@Component
@EnableJms
public class JmsConsumerStudent implements MessageListener {

    @Autowired
    GatewayController gatewayController;
    @Override
    @JmsListener(destination = "${active-mq.student-resp-topic}")
    public void onMessage(Message message) {
        try{

            String text = ((ActiveMQTextMessage) message).getText();
            //do additional processing
            System.out.println("Received student resp Message from Topic: "+ text);
            gatewayController.setStudentResp(text);
